...
def visualize_routes(missions, routes, uavs_end, uavs_speeds, num_uavs, num_missions):
    ...
    plt.scatter(unique_end[0], unique_end[1], c='red', marker='X', s=100, label='End Point')
    plt.text(unique_end[0] + 0.5, unique_end[1] + 0.5, "End Point", fontsize=9, color='red')
...
if __name__ == "__main__":
    # 데이터 초기화
    mission_data = MissionData(num_missions=20, num_uavs=3, seed=42, device='cpu')  # 미션 수를 줄여 계산 시간을 단축
