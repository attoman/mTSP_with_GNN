...
def compute_reward_max_time(env, max_possible_time=100000, use_2opt=True):
    """
    최대 이동 시간을 기반으로 보상 계산, 선택적으로 2-opt 최적화 적용

    Args:
        env (MissionEnvironment): 환경 인스턴스
        max_possible_time (float): 스케일링을 위한 최대 가능 시간
        use_2opt (bool): 2-opt 최적화를 적용할지 여부
...
class EnhancedGNNTransformerEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        # GNN 레이어 처리
        current_features = x
        for conv, proj in zip(self.gnn_layers, self.residual_projections):
            print(f"current_features shape before proj: {current_features.shape}")  # 디버깅용
            residual = proj(current_features)
            print(f"residual shape after proj: {residual.shape}")  # 디버깅용
            conv_out = conv(current_features, edge_index)
            conv_out = F.relu(conv_out)
            conv_out = F.dropout(conv_out, p=self.dropout_prob, training=self.training)
            print(f"conv_out shape: {conv_out.shape}")  # 디버깅용
            current_features = conv_out + residual
            print(f"current_features shape after addition: {current_features.shape}")  # 디버깅용
...
class ImprovedActorCriticNetwork(nn.Module):
    def __init__(self, num_missions, num_uavs, embedding_dim=64, gnn_hidden_dim=64,
        ...
        self.feature_fusion = nn.Sequential(
            nn.Linear(gnn_hidden_dim * heads + embedding_dim * 2, 192),  # gnn_hidden_dim * heads + time_encoded + spatial_encoded
            nn.ReLU(),
            nn.LayerNorm(192),
        ...
        self.output_layer = nn.Sequential(
            nn.Linear(192, num_missions),  # in_features=192으로 수정
            nn.LayerNorm(num_missions),
            nn.Dropout(gnn_dropout)
        )

        # Actor 네트워크
        self.actor_shared = nn.Sequential(
            nn.Linear(192, actor_hidden_dim),
            nn.ReLU(),
        ...
        self.critic_shared = nn.Sequential(
            nn.Linear(192, critic_hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, mission_coords, edge_index, batch, uavs_info, action_mask,
        ...
        num_uavs = speeds.size(0)
        num_missions = mission_coords.size(0)
        
        # 1. 시간 관련 특징 준비
        time_features = torch.cat([
            timetogo_matrix.unsqueeze(-1),
            torch.zeros_like(timetogo_matrix.unsqueeze(-1)),
            torch.zeros_like(timetogo_matrix.unsqueeze(-1))
        ], dim=-1)  # (num_uavs, num_missions, 3)
        
        # 2. 시간 특징 인코딩
        time_encoded = self.time_encoder(time_features)  # (num_uavs, num_missions, embedding_dim)
        
        # 3. mission_coords 차원 확장
        mission_coords = mission_coords.unsqueeze(0).repeat(num_uavs, 1, 1)  # (num_uavs, num_missions, 2)
        print(f"mission_coords shape after repeat: {mission_coords.shape}")
        
        # 4. 공간 특징 인코딩
        spatial_encoded = self.spatial_encoder(mission_coords)  # (num_uavs, num_missions, embedding_dim)
        print(f"spatial_encoded shape: {spatial_encoded.shape}")
        
        # 5. GNN 입력 특징 준비
        x = torch.cat([
            mission_coords,  # (num_uavs, num_missions, 2)
            action_mask.unsqueeze(-1).float(),  # (num_uavs, num_missions, 1)
            speeds.unsqueeze(-1).unsqueeze(1).repeat(1, num_missions, 1),  # (num_uavs, num_missions, 1)
            dist_matrix.unsqueeze(-1),  # (num_uavs, num_missions, 1)
            timetogo_matrix.unsqueeze(-1),  # (num_uavs, num_missions, 1)
            time_encoded,  # (num_uavs, num_missions, embedding_dim)
            spatial_encoded  # (num_uavs, num_missions, embedding_dim)
        ], dim=-1)  # (num_uavs, num_missions, 2+1+1+1+1+embedding_dim*2)
        print(f"x shape before GNN encoding: {x.shape}")
        
        # Flatten 배치와 노드 차원
        x = x.view(-1, x.size(-1))  # (num_uavs * num_missions, features)
        print(f"x shape after flatten: {x.shape}")
        
        # 6. GNN 인코딩
        x = self.gnn_encoder(x, edge_index, batch)  # (num_nodes, encoding_dim)
        print(f"GNN Encoder output shape: {x.shape}")
        
        # 7. 특징 결합 및 융합
        combined_features = torch.cat([
            x,  # (num_nodes, gnn_hidden_dim * heads)
            time_encoded.view(-1, time_encoded.size(-1)),  # (num_nodes, embedding_dim)
            spatial_encoded.view(-1, spatial_encoded.size(-1))  # (num_nodes, embedding_dim)
        ], dim=-1)  # (num_nodes, gnn_hidden_dim * heads + embedding_dim * 2)
        print(f"combined_features shape: {combined_features.shape}")
        
        # 8. 특징 결합 레이어 통과
        fused_features = self.feature_fusion(combined_features)  # (num_nodes, 192)
        print(f"fused_features shape: {fused_features.shape}")
        
        # 9. 출력 레이어 통과
        output = self.output_layer(fused_features)  # (num_nodes, num_missions)
        print(f"output shape: {output.shape}")
        
        # Actor 네트워크
        actor_hidden = self.actor_shared(fused_features)  # (num_nodes, actor_hidden_dim)
        action_logits = self.actor_out(actor_hidden)  # (num_nodes, num_missions)
        print(f"action_logits shape: {action_logits.shape}")
        
        # Critic 네트워크
        critic_hidden = self.critic_shared(fused_features)  # (num_nodes, critic_hidden_dim)
        state_values = self.critic_out(critic_hidden).squeeze(-1)  # (num_nodes,)
...
def train_model(env, val_env, policy_net, optimizer_actor, optimizer_critic, scheduler_actor, scheduler_critic,
    ...
    wandb.init(project="multi_uav_mission", name=wandb_name, config={
        ...
        "lr_gamma": lr_gamma,
        "patience": patience,
        "gnn_dropout": get_policy_module(policy_net).p,
        ...
        "gnn_hidden_dim": get_policy_module(policy_net).gnn_encoder.gnn_layers[0].out_channels // get_policy_module(policy_net).gnn_encoder.heads,
            ...
            for batch_idx in epoch_pbar:
                ...
                while not done:
                    ...
                    action_logits, state_values = get_policy_module(policy_net).forward(
                        env.missions,       # (num_missions, 2)
                        edge_index, 
                        batch, 
                        uavs_info, 
                        action_mask, 
                        env.speeds,        # (num_uavs,)
                        dist_matrix,       # (num_uavs, num_missions)
                        timetogo_matrix    # (num_uavs, num_missions)
                    )

                    # 텐서 차원 출력
                    print(f"action_logits shape: {action_logits.shape}, state_values shape: {state_values.shape}")
                    ...
                    for i, action in enumerate(actions):
                        if action != -1:
                            ...
                            # 텐서 차원 출력
                            print(f"log_prob shape: {log_prob.shape}, value shape: {state_values[i].squeeze().shape}")
                ...
                policy_loss = []
                value_loss = []
                for log_prob, value, R_tensor in zip(log_probs, values, returns):
                    advantage = R_tensor - value
                    policy_loss.append(-log_prob * advantage)
                    value_loss.append(F.mse_loss(value, R_tensor))
...
def validate_model(env, policy_net, device, edge_index, batch, checkpoints_path, results_path, epoch, reward_type, alpha, beta, gamma, wandb_name, use_2opt=False):
    """
    정책 네트워크를 검증합니다.
...
def test_model(env, policy_net, device, edge_index, batch, checkpoint_path, results_path, reward_type, alpha, beta, gamma, wandb_name="run", use_2opt=False):
    ...
    try:
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            print(f"체크포인트 '{checkpoint_path}'를 로드했습니���. 테스트를 시작합니다.")
        else:
...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=0.7, help="최대 이동 시��� 가중치 ('mixed'에서 사용)")
    ...
    parser.add_argument('--use_2opt', action='store_true', help="훈련에서 3-opt 최적화 사용 여부")
    ...
    # 옵티마이저 초기화
    model = get_policy_module(policy_net)
    actor_params = list(model.actor_shared.parameters()) + list(model.actor_out.parameters())
    critic_params = list(model.critic_shared.parameters()) + list(model.critic_out.parameters())
