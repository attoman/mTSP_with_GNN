...
def compute_reward_max_time(env, max_possible_time=1000, use_3opt=True):
    """
    3-opt 최적화를 적용하여 최대 이동 시간이 작을수록 높은 보상이 주어지도록 보상을 계산합니다.
    
    Args:
        env (MissionEnvironment): 환경 인스턴스.
        max_possible_time (float): 보상의 상한 설정.
        use_3opt (bool): 3-opt 최적화 적용 여부.
...
def compute_reward_total_time(env, max_possible_time=1000, use_3opt=True):
    """
    3-opt 최적화를 적용하여 총 이동 시간이 작을수록 높은 보상이 주어지도록 보상을 계산합니다.
...
class MissionEnvironment:
    ...
    def __init__(self, missions=None, uavs_start=None, uavs_speeds=None, device='cpu', mode='train', seed=None, time_weight=2.0, use_3opt=False):
        ...
        self.use_3opt = use_3opt  # use_3opt 인자를 저장
    ...
    def step(self, actions):
        ...
        # 3-opt 알고리즘 적용 (플래그가 활성화된 경우)
        if self.use_3opt:
            for i in range(self.num_uavs):
                self.paths[i] = apply_3opt(self.paths[i], self.missions)
...
def validate_model(env, policy_net, device, edge_index, batch, checkpoints_path, results_path, epoch, reward_type, alpha, beta, gamma, wandb_name="run", use_3opt=False):
    """
    정책 네트워크를 검증합니다.
...
def main():
    ...
    # 3-opt 사용 여부 추가
    parser.add_argument('--use_3opt', action='store_true', help="3-opt 알고리즘을 학습에 포함 여부 확인")
